...
const Dashboard: React.FC<DashboardProps> = ({ items, aiSummary, loadingSummary, onRefreshSummary }) => {
  const stats = useMemo(() => {
  ...
  const ControlWidget = ({ title, value, subValue, icon: Icon, colorClass, bgGradient, delay }: any) => (
    <div 
        className={`glass-panel rounded-[2rem] p-5 relative overflow-hidden group hover:bg-white/10 transition-all duration-500 hover:scale-[1.02] cursor-pointer shadow-lg backdrop-blur-xl animate-fade-in`}
  ...
  const renderAiSummary = () => {
    if (!aiSummary) return "กดปุ่ม 'รีเฟรชข้อมูล' เพื่อให้ AI ช่วยสรุปภาพรวมงานของคุณ...";
    
    return aiSummary.split(/(\*\*.*?\*\*)/).map((part, index) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        const content = part.slice(2, -2);
        return (
          <span key={index} className="font-bold text-yellow-300 drop-shadow-sm px-1">
            {content}
          </span>
        );
      }
  ...
  return (
    <div className="space-y-6">
      {/* Top Grid (Bento Box Style) */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        <ControlWidget 
            title="Completed" 
            value={stats.completed} 
            subValue={`${Math.round((stats.completed/stats.total || 0)*100)}%`} 
            icon={CheckCircle2} 
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-zinc-800 dark:text-white">AI Weekly Insight</h3>
                            <p className="text-sm text-zinc-500 dark:text-white/60">Powered by Gemini 2.5</p>
                        </div>
                    </div>
                    <button 
                        onClick={onRefreshSummary}
                        className="flex items-center gap-2 text-xs bg-white/10 hover:bg-white/20 text-indigo-600 dark:text-white px-5 py-2.5 rounded-full shadow-sm transition-all border border-white/10 backdrop-blur-md"
                    >
                        {loadingSummary ? <Loader2 size={16} className="animate-spin" /> : <Zap size={16} className="fill-current" />}
                        {loadingSummary ? 'Analyzing...' : 'Refresh'}
                    </button>
                </div>
          ...
          <div className="glass-panel rounded-[2.5rem] p-6 flex flex-col shadow-xl animate-fade-in" style={{animationDelay: '500ms'}}>
            <div className="flex items-center gap-3 mb-6">
                <div className="bg-blue-500/20 p-2 rounded-xl">
                    <TrendingUp className="text-blue-500 dark:text-blue-300" size={20} />
                </div>
                <h3 className="font-bold text-zinc-800 dark:text-white">Categories</h3>
            </div>
